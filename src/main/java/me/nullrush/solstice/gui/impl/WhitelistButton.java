...
import me.nullrush.solstice.settings.impl.WhitelistSetting;
...
public class WhitelistButton extends Button {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Solstice.FONT_MANAGER.drawTextWithShadow(context, Formatting.GRAY + "" + setting.getWhitelist().size(), getX() + getWidth() - getTextPadding() - 1 - Solstice.FONT_MANAGER.getWidth(setting.getWhitelist().size() + ""), getY() + 2, Color.WHITE);

        if (open) {
            int contentY = getY() + getParent().getHeight();

            renderRow(context, mouseX, mouseY, searching, contentY);

            String displayedSearch = searchQuery;
            if (searching) {
                String cursorChar = Solstice.CLICK_GUI.isShowLine() ? "|" : " ";
            ...
            Solstice.FONT_MANAGER.drawTextWithShadow(context, displayedSearch, getX() + getTextPadding() + 2 + offsetX, contentY + (searchBarHeight / 2) - (Solstice.FONT_MANAGER.getHeight() / 2), searching ? (selecting ? ClickGuiScreen.getButtonColor(getY(), 255) : Color.WHITE) : Color.GRAY);
            ...
            for (int i = 0; i < renderCount; i++) {
                ...
                renderRow(context, mouseX, mouseY, inWhitelist, itemY);
                ...
                for (ColoredSegment seg : segments) {
                    textWidth += Solstice.FONT_MANAGER.getWidth(seg.text);
                }
                ...
                for (ColoredSegment seg : drawSegments) {
                    Solstice.FONT_MANAGER.drawTextWithShadow(context, seg.text, drawX, itemY + 2, seg.color);
                    drawX += Solstice.FONT_MANAGER.getWidth(seg.text);
                }
                ...
                Solstice.FONT_MANAGER.drawTextWithShadow(context, symbol, symbolRenderX, itemY + 2, symbolColor);
            }
        ...
        if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
            ...
            if (keyCode == GLFW.GLFW_KEY_V) {
                try {
                    ...
                    Solstice.LOGGER.error("{}: Failed to process clipboard paste", e.getClass().getName(), e);
                }
            } else if (keyCode == GLFW.GLFW_KEY_C && selecting) {
                try {
                    mc.keyboard.setClipboard(searchQuery);
                } catch (Exception e) {
                    Solstice.LOGGER.error("{}: Failed to process clipboard change", e.getClass().getName(), e);
                }
    ...
    private List<ColoredSegment> truncateSegments(List<ColoredSegment> segments, int maxWidth) {
        ...
        for (ColoredSegment seg : segments) {
            int segWidth = Solstice.FONT_MANAGER.getWidth(seg.text);
            if (currentWidth + segWidth <= maxWidth) {
                ...
                if (spaceLeft > 0) {
                    ...
                    for (int i = 0; i < seg.text.length(); i++) {
                        ...
                        int charWidth = Solstice.FONT_MANAGER.getWidth(String.valueOf(c));
                        if (charWidth <= spaceLeft - Solstice.FONT_MANAGER.getWidth("…")) {
                            sb.append(c);
                            spaceLeft -= charWidth;
                        } else {
                            break;
