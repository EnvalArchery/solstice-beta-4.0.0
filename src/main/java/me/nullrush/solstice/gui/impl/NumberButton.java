...
public class NumberButton extends Button {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        renderRow(context, mouseX, mouseY, dragging || listening, getY());
        Renderer2D.renderQuad(context.getMatrices(), getX() + getPadding() + 1, getY() + getHeight() - 4, getX() + getPadding() + 1 + (float) slider, getY() + getHeight() - 1, ClickGuiScreen.getButtonColor(getY(), 165));

        Solstice.FONT_MANAGER.drawTextWithShadow(context, listening ? (currentString + (selecting ? "" : Solstice.CLICK_GUI.isShowLine() ? "|" : "")) : setting.getTag(), getX() + getTextPadding() + 1, getY() + 2, Color.WHITE);
        if(!listening) Solstice.FONT_MANAGER.drawTextWithShadow(context, Formatting.GRAY + "" + setting.getValue(), getX()+ getWidth() - getTextPadding() - 1 - Solstice.FONT_MANAGER.getWidth(setting.getValue() + ""), getY() + 2, Color.WHITE);
    }
    ...
    public void keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (InputUtil.isKeyPressed(mc.getWindow().getHandle(), GLFW.GLFW_KEY_ENTER)) {
            try {
                ...
                Solstice.CHAT_MANAGER.warn("Please input a valid " + ChatUtils.getPrimary() + setting.getType().name().toLowerCase() + ChatUtils.getSecondary() + " number.");
            }
            selecting = false;
            listening = false;
            return;
