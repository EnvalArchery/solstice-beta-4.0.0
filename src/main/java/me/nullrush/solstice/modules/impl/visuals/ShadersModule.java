...
public class ShadersModule extends Module {
    ...
    public ModeSetting style = new ModeSetting("Style", "The shader style that will be used on rendered targets.", "Classic", new String[]{"Classic", "Soft", "Neon", "Bloom", "Pulse", "Scanner", "Ghost", "Static"});
    ...
    public NumberSetting outlineWidth = new NumberSetting("OutlineWidth", "Width", "The width of the outline glow.", 3.0f, 1.0f, 8.0f);
    public NumberSetting feather = new NumberSetting("Feather", "Feather", "The softness of the outline fade.", 5.0f, 1.0f, 16.0f);
    public NumberSetting brightness = new NumberSetting("Brightness", "Brightness", "The brightness multiplier used by the shader.", 1.0f, 0.5f, 2.5f);
    public NumberSetting pulseSpeed = new NumberSetting("PulseSpeed", "PulseSpeed", "The speed used by animated shader styles.", new ModeSetting.Visibility(style, "Pulse", "Scanner", "Static"), 8.0f, 1.0f, 20.0f);
    public NumberSetting pulseAmount = new NumberSetting("PulseAmount", "PulseAmount", "The intensity used by animated shader styles.", new ModeSetting.Visibility(style, "Pulse", "Scanner", "Ghost", "Static"), 0.35f, 0.0f, 1.5f);
    ...
    public void onRenderShader(RenderShaderEvent event) {
        ...
        Solstice.SHADER_MANAGER.prepare();
    }
    ...
    public void onRenderEntity(RenderEntityEvent event) {
        ...
        Color color = (event.getEntity() instanceof PlayerEntity player && Solstice.FRIEND_MANAGER.contains(player.getName().getString()) && !friends.getValue().equals("Sync")) ? (friends.getValue().equals("Default") ? Solstice.FRIEND_MANAGER.getDefaultFriendColor() : friendColor.getColor()) : this.color.getColor();
        event.setVertexConsumers(Solstice.SHADER_MANAGER.create(event.getVertexConsumers(), color));
    }
    ...
    public void onRenderEntity$POST(RenderEntityEvent.Post event) {
        ...
        if (players.getValue() || hostiles.getValue() || animals.getValue() || ambient.getValue() || invisibles.getValue() || items.getValue() || crystals.getValue() || others.getValue()) {
            Solstice.SHADER_MANAGER.getVertexConsumerProvider().draw();
        }

        if (!hands.getValue()) Solstice.SHADER_MANAGER.render(getRenderMode(), opacity.getValue().intValue() / 255.0f, getShaderStyle(), outlineWidth.getValue().floatValue(), feather.getValue().floatValue(), brightness.getValue().floatValue(), pulseSpeed.getValue().floatValue(), pulseAmount.getValue().floatValue(), color.getColor());
    }
    ...
    public void onRenderHand(RenderHandEvent event) {
        ...
        event.setVertexConsumers(Solstice.SHADER_MANAGER.create(event.getVertexConsumers(), color.getColor()));
    }
    ...
    public void onRenderShader$POST(RenderShaderEvent.Post event) {
        ...
        Solstice.SHADER_MANAGER.render(getRenderMode(), opacity.getValue().intValue() / 255.0f, getShaderStyle(), outlineWidth.getValue().floatValue(), feather.getValue().floatValue(), brightness.getValue().floatValue(), pulseSpeed.getValue().floatValue(), pulseAmount.getValue().floatValue(), color.getColor());
    }

    private int getRenderMode() {
        return mode.getValue().equalsIgnoreCase("Fill") ? 0 : mode.getValue().equalsIgnoreCase("Outline") ? 1 : 2;
    }

    private int getShaderStyle() {
