    ...
    public class Action {
        ...
        public boolean process() {
            ...
            if (mining) {
                ...
                float delta = WorldUtils.getMineSpeed(this.state, slot) / Solstice.WORLD_MANAGER.getTimerMultiplier();
                ...
                if (rotate.getValue().equalsIgnoreCase("Normal") && progress + (delta * 2) >= getSpeed()) {
                    Solstice.ROTATION_MANAGER.rotate(RotationUtils.getRotations(WorldUtils.getHitVector(position, direction)), Solstice.ROTATION_MANAGER.getModulePriority(Solstice.MODULE_MANAGER.getModule(SpeedMineModule.class)));
                }
                ...
                if (progress >= getSpeed() && !state.isReplaceable() && (whileEating.getValue() || !mc.player.isUsingItem())) {
                    if (!instantMine || instantTimer.hasTimeElapsed(instantDelay.getValue().longValue() * 50L)) {
                        Solstice.EVENT_HANDLER.post(new DestroyBlockEvent(position));

                        if (rotate.getValue().equalsIgnoreCase("Packet")) Solstice.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(WorldUtils.getHitVector(position, direction)));
