...
import me.nullrush.solstice.events.impl.UpdateMovementEvent;
import me.nullrush.solstice.modules.Module;
import me.nullrush.solstice.modules.RegisterModule;
import me.nullrush.solstice.modules.impl.core.AntiCheatModule;
...
public class AuraModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        Iterable<Entity> entities = mc.world.getOtherEntities(mc.player, PositionUtils.getRadius(mc.player, Math.max(range.getValue().doubleValue(), wallsRange.getValue().doubleValue()) + 1.0), entity ->
                entity.isAlive()
                        ...
                        && (friends.getValue() || !Solstice.FRIEND_MANAGER.contains(entity.getName().getString()))
        ...
        if (rotate.getValue().equalsIgnoreCase("Hold")) Solstice.ROTATION_MANAGER.rotate(RotationUtils.getRotations(target), this);

        attacking = true;

        if (hitDelay.getValue().equalsIgnoreCase("Vanilla") && !Solstice.MODULE_MANAGER.getModule(AntiCheatModule.class).canAttack()) return;
        if (hitDelay.getValue().equalsIgnoreCase("Custom") && !timer.hasTimeElapsed(1000.0f - speed.getValue().floatValue() * 50.0f))
            return;

        if (rotate.getValue().equalsIgnoreCase("Normal")) Solstice.ROTATION_MANAGER.rotate(RotationUtils.getRotations(target), this);
    ...
    public void onUpdateMovement$POST(UpdateMovementEvent.Post event) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Packet")) Solstice.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(target));
