...
import me.nullrush.solstice.events.impl.*;
import me.nullrush.solstice.modules.Module;
import me.nullrush.solstice.modules.RegisterModule;
import me.nullrush.solstice.modules.impl.player.SpeedMineModule;
import me.nullrush.solstice.modules.impl.player.AutoXPModule;
import me.nullrush.solstice.settings.impl.*;
import me.nullrush.solstice.utils.color.ColorUtils;
import me.nullrush.solstice.utils.minecraft.DamageUtils;
...
import me.nullrush.solstice.utils.system.Counter;
...
public class AutoCrystalModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        attackedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > Solstice.SERVER_MANAGER.getPing() * 2L);
        placedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > Solstice.SERVER_MANAGER.getPing() * 2L + (20 - attackSpeed.getValue().floatValue()) * 50L);
        countedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > Solstice.SERVER_MANAGER.getPing() * 2L);
        ...
        Runnable runnable = () -> {
            ...
            if (blockDestruction.getValue() && asynchronous.getValue()) {
                SpeedMineModule module = Solstice.MODULE_MANAGER.getModule(SpeedMineModule.class);
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Packet")) Solstice.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)));
        if (rotate.getValue().equalsIgnoreCase("Normal")) Solstice.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)), Solstice.ROTATION_MANAGER.getModulePriority(this));
    ...
    public void onDestroyBlock(DestroyBlockEvent event) {
        ...
        if (mineTarget == null || mineTarget.getPosition() == null) {
            Solstice.RENDER_MANAGER.setRenderPosition(null);
            return;
        }

        BlockPos position = mineTarget.getPosition();
        Solstice.RENDER_MANAGER.setRenderPosition(position);
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) Solstice.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), this, Solstice.ROTATION_MANAGER.getModulePriority(this) + 1);
        if (!rotate.getValue().equalsIgnoreCase("None")) Solstice.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
        ...
        for (Entity entity : mc.world.getOtherEntities(null, new Box(position.up())).stream().filter(entity -> entity instanceof EndCrystalEntity).toList()) {
            if (!rotate.getValue().equalsIgnoreCase("None")) Solstice.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(entity));
    ...
    private void placeCrystals(boolean sequential) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) Solstice.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), Solstice.ROTATION_MANAGER.getModulePriority(this));
        ...
        placeRunnable = () -> {
            ...
            if (rotate.getValue().equalsIgnoreCase("Packet")) Solstice.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
            ...
            if (godSync.getValue()) {
        boolean flag = !antiKick.getValue() || !(mc.player.getMainHandStack().getItem() instanceof ExperienceBottleItem) && !(mc.player.getOffHandStack().getItem() instanceof ExperienceBottleItem) && !Solstice.MODULE_MANAGER.getModule(AutoXPModule.class).isToggled();
    ...
    private EndCrystalEntity calculateCrystals() {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (!Solstice.MODULE_MANAGER.getModule(SuicideModule.class).isToggled()) {
    ...
    private PlaceTarget calculatePlacements(BlockPos exception) {
        ...
        for (int i = 0; i < Solstice.WORLD_MANAGER.getRadius(Math.max(placeRange.getValue().doubleValue(), placeWallsRange.getValue().doubleValue())); i++) {
    ...
    private float[] calculateRotations(Vec3d vec3d) {
        ...
        if (yawStep.getValue()) {
            ...
            float difference = Solstice.ROTATION_MANAGER.getServerYaw() - rotations[0];
            ...
            if (Math.abs(difference) > yawStepThreshold.getValue().floatValue()) yaw = Solstice.ROTATION_MANAGER.getServerYaw() + deltaYaw;
