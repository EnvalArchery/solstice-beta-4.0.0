...
import me.nullrush.solstice.mixins.accessors.PlayerMoveC2SPacketAccessor;
...
public class ElytraFlyModule extends Module {
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        ElytraTargetModule elytraTarget = Solstice.MODULE_MANAGER.getModule(ElytraTargetModule.class);
        boolean assistingTarget = elytraTarget.isToggled() && elytraTarget.shouldAssist();
        if (assistingTarget) {
            Vec3d velocity = elytraTarget.getChaseVelocity();
            event.setX(velocity.x);
            event.setY(velocity.y);
            event.setZ(velocity.z);
            pitch = elytraTarget.getTargetPitch();
        }
        ...
        switch (ncpStrict.getValue().toLowerCase()) {
            case "old" -> event.setY((assistingTarget ? event.getY() : 0.0) + 0.0002 - (mc.player.age % 2 == 0 ? 0 : 0.000001) + MathUtils.random(0.0000009, 0));
            case "new" -> event.setY(assistingTarget ? Math.min(event.getY(), -1.000088900582341E-12) : -1.000088900582341E-12);
            case "motion" -> event.setY(assistingTarget ? Math.min(event.getY(), -4.000355602329364E-12) : -4.000355602329364E-12);
        }
        ...
        if (!assistingTarget && (infiniteDurability.getValue() || ncpStrict.getValue().equalsIgnoreCase("Motion"))) {
    ...
    public void onPlayerTravel(PlayerTravelEvent event) {
        ...
        if (mode.getValue().equalsIgnoreCase("Control")) {
            ElytraTargetModule elytraTarget = Solstice.MODULE_MANAGER.getModule(ElytraTargetModule.class);
            if (elytraTarget.isToggled() && elytraTarget.shouldAssist()) {
                event.setCancelled(true);
