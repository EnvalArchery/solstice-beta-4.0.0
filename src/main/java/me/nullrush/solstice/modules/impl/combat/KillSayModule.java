...
public class KillSayModule extends Module {
    private final String[] KILL_MESSAGES = {"Sit [username], Solstice owns me and all!", "[username] I killed you with the power of Solstice!", "Sorry [username], get good get Solstice!", "I just killed [username] thanks to Solstice!"};
    ...
    public void onTick(TickEvent event) {
        ...
        killMessages = FileUtils.readLines(new File(Solstice.MOD_NAME + "/Client/" + killFile.getValue()));
        popMessages = FileUtils.readLines(new File(Solstice.MOD_NAME + "/Client/" + popFile.getValue()));

        synchronized (queue) {
            if (timer.hasTimeElapsed(delay.getValue().intValue() * 1000)) {
                String message = queue.poll();
                if(!message.isEmpty()) {
                    if (clientside.getValue()) Solstice.CHAT_MANAGER.message(message);
    ...
    public void onTargetDeath(TargetDeathEvent event) {
        ...
        if(killStreak.getValue() && streak > 1) {
            Solstice.CHAT_MANAGER.message("You are on a " + streak + " kill streak.");
        }
    }

    @SubscribeEvent
    public void onPlayerPop(PlayerPopEvent event) {
        if(getNull() || event.getPlayer() == mc.player || !Solstice.TARGET_MANAGER.isTarget(event.getPlayer()) || !pops.getValue()) return;
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        if(event.getPlayer() == mc.player && (!suicideIgnore.getValue() || !suicide && !Solstice.MODULE_MANAGER.getModule(SuicideModule.class).isToggled())) {
            streak = 0;
        }
