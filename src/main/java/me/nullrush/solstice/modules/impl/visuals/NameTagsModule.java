...
import me.nullrush.solstice.mixins.accessors.ItemRenderStateAccessor;
import me.nullrush.solstice.mixins.accessors.ItemRendererAccessor;
import me.nullrush.solstice.mixins.accessors.LayerRenderStateAccessor;
import me.nullrush.solstice.modules.Module;
import me.nullrush.solstice.modules.RegisterModule;
import me.nullrush.solstice.modules.impl.miscellaneous.FakePlayerModule;
...
import me.nullrush.solstice.utils.text.CustomFormatting;
...
public class NameTagsModule extends Module {
    ...
    public BooleanSetting solsticeCheck = new BooleanSetting("SolsticeCheck", "Adds an indicator next to the name of other users..", true);
    ...
    public void onRenderWorld(RenderWorldEvent.Post event) {
        ...
        for (PlayerEntity player : mc.world.getPlayers().stream().sorted(Comparator.comparing(p -> -mc.player.distanceTo(p))).toList()) {
            ...
            int width = Solstice.FONT_MANAGER.getWidth(text);

            if (border.getValue().equalsIgnoreCase("Fill") || border.getValue().equalsIgnoreCase("Both")) Renderer2D.renderQuad(matrices, -width / 2.0f - 1, -Solstice.FONT_MANAGER.getHeight() - 1, width / 2.0f + 2, 0, fillColor.getColor());
            if (border.getValue().equalsIgnoreCase("Outline") || border.getValue().equalsIgnoreCase("Both")) Renderer2D.renderOutline(matrices, -width / 2.0f - 1, -Solstice.FONT_MANAGER.getHeight() - 1, width / 2.0f + 2, 0, outlineColor.getColor());

            Solstice.FONT_MANAGER.drawTextWithShadow(matrices, text, -width / 2, -Solstice.FONT_MANAGER.getHeight(), vertexConsumers, Solstice.MODULE_MANAGER.getModule(FakePlayerModule.class).isToggled() && Solstice.MODULE_MANAGER.getModule(FakePlayerModule.class).getPlayer() == player ? new Color(225, 0, 70) : player.isSneaking() ? new Color(255, 170, 0) : Solstice.FRIEND_MANAGER.contains(player.getName().getString()) ? Solstice.FRIEND_MANAGER.getDefaultFriendColor() : Color.WHITE);
            ...
            if (enchantments.getValue()) {
                for (int i = 0; i < 6; i++) {
                    ...
                    if (!component.getEnchantments().isEmpty()) {
                        int height = (component.getEnchantments().size() * Solstice.FONT_MANAGER.getHeight() / 2) - 18;
            ...
            for (int i = 0; i < 6; i++) {
                ...
                int stackY = -Solstice.FONT_MANAGER.getHeight() - 1 - (items.getValue() ? 18 + maxEnchants : 1);
                ...
                if (items.getValue()) {
                    ...
                    if (stack.getItem().equals(Items.ENCHANTED_GOLDEN_APPLE)) {
                        ...
                        Solstice.FONT_MANAGER.drawTextWithShadow(matrices, "God", 0, 0, vertexConsumers, new Color(255, 125, 255));
                        matrices.pop();
                    }
                    ...
                    if (stack.getCount() != 1) {
                        ...
                        matrices.push();
                        matrices.translate(stackX + 17 - Solstice.FONT_MANAGER.getWidth(count), stackY + 9, 0);
                        Solstice.FONT_MANAGER.drawTextWithShadow(matrices, count, 0, 0, vertexConsumers, Color.WHITE);
                        matrices.pop();
                    }
                }
                ...
                if (durability.getValue() && stack.isDamageable()) {
                    ...
                    matrices.push();
                    matrices.translate(stackX, stackY - Solstice.FONT_MANAGER.getHeight() / 2f - 1, 0);
                    matrices.scale(0.5f, 0.5f, 1);
                    Solstice.FONT_MANAGER.drawTextWithShadow(matrices, Math.round(((stack.getMaxDamage() - stack.getDamage()) * 100.0f) / stack.getMaxDamage()) + "%", 0, 0, vertexConsumers, new Color(red, green, 0));
                    matrices.pop();
                ...
                if (items.getValue() && enchantments.getValue() && stack.hasEnchantments()) {
                    ...
                    for (Object2IntMap.Entry<RegistryEntry<Enchantment>> entry : Object2IntMaps.fastIterable(enchantments)) {
                        ...
                        matrices.scale(0.5f, 0.5f, 1);
                        Solstice.FONT_MANAGER.drawTextWithShadow(matrices, str, 0, 0, vertexConsumers, Color.WHITE);
                        matrices.pop();

                        height += Solstice.FONT_MANAGER.getHeight() / 2;
                    }
                }
            }
            ...
            if (itemName.getValue() && !player.getMainHandStack().isEmpty()) {
                ...
                matrices.push();
                matrices.translate(-Solstice.FONT_MANAGER.getWidth(itemText) / 2f / 2f, -Solstice.FONT_MANAGER.getHeight() - 1 - Solstice.FONT_MANAGER.getHeight() / 2f - 1 - (renderedItems ? (items.getValue() ? 18 + maxEnchants : 1) + (durability.getValue() && renderedDurability ? Solstice.FONT_MANAGER.getHeight() / 2.0f + 1 : 0) : 0), 0);
                matrices.scale(0.5f, 0.5f, 1);
                Solstice.FONT_MANAGER.drawTextWithShadow(matrices, itemText, 0, 0, vertexConsumers, Color.WHITE);
                matrices.pop();
            }
