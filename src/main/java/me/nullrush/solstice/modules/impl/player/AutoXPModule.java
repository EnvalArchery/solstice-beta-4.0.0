...
import me.nullrush.solstice.utils.minecraft.NetworkUtils;
...
@RegisterModule(name = "AutoXP", description = "Automatically switches to experience bottles and throws them.", category = Module.Category.PLAYER)
public class AutoXPModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (autoSwitch.getValue().equalsIgnoreCase("None") && !(mc.player.getMainHandStack().getItem() instanceof BlockItem)) {
            Solstice.CHAT_MANAGER.tagged("You are currently not holding any experience bottles.", getName());
            setToggled(false);
            return;
        }
        ...
        if (slot == -1) {
            Solstice.CHAT_MANAGER.tagged("No experience bottles could be found in your hotbar.", getName());
            setToggled(false);
            return;
        }
