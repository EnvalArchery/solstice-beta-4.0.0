...
public class PingSpoofModule extends Module {
    ...
    public void onEnable() {
        if (Solstice.MODULE_MANAGER.getModule(FastLatencyModule.class).isToggled()) setToggled(false);
    }

    @SubscribeEvent
    public void onPacketReceive(PacketReceiveEvent event) {
        if (getNull() || Solstice.MODULE_MANAGER.getModule(FastLatencyModule.class).isToggled()) return;
