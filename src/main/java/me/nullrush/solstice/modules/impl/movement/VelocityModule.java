...
import me.nullrush.solstice.mixins.accessors.EntityVelocityUpdateS2CPacketAccessor;
...
public class VelocityModule extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        if (mode.getValue().equalsIgnoreCase("Grim") && (!pause.getValue() || Solstice.SERVER_MANAGER.getSetbackTimer().hasTimeElapsed(100L))) {
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), Solstice.ROTATION_MANAGER.getServerYaw(), Solstice.ROTATION_MANAGER.getServerPitch(), mc.player.isOnGround(), mc.player.horizontalCollision));
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (event.getPacket() instanceof EntityVelocityUpdateS2CPacket packet) {
            ...
            switch (mode.getValue()) {
                ...
                case "Cancel" -> {
                    if (pause.getValue() && !Solstice.SERVER_MANAGER.getSetbackTimer().hasTimeElapsed(100L)) return;
