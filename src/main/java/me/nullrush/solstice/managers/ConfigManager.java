...
public class ConfigManager {
    ...
    public void loadConfig() {
        try {
            FileUtils.createDirectory(Solstice.MOD_NAME);
            FileUtils.createDirectory(Solstice.MOD_NAME + "/Configs");
            FileUtils.createDirectory(Solstice.MOD_NAME + "/Client");

            loadGeneral();
            loadWaypoints();
            loadModules(currentConfig);
        } catch (IOException exception) {
            Solstice.LOGGER.error("Failed to load the client's configuration!", exception);
            Solstice.CHAT_MANAGER.await("The configuration has not been loaded properly. Read the stacktrace for more information.");
        }
    }
    ...
    public void saveConfig() {
        try {
            ...
            Solstice.LOGGER.error("Failed to save the client's configuration!", exception);
        }
    }

    public void loadWaypoints() throws IOException {
        if (!FileUtils.fileExists(Solstice.MOD_NAME + "/Waypoints.json")) return;
        @Cleanup InputStream stream = Files.newInputStream(Paths.get(Solstice.MOD_NAME + "/Waypoints.json"));

        JsonObject configObject;
        try {
            configObject = JsonParser.parseReader(new InputStreamReader(stream)).getAsJsonObject();
        } catch (IllegalStateException exception) {
            Solstice.LOGGER.error("Failed to load the client's Waypoint configuration!", exception);
            Solstice.CHAT_MANAGER.await("The Waypoint configuration has not been loaded properly. Read the stacktrace for more information.");
            return;
        }
        ...
        if(configObject.has("Waypoints")) {
            for(JsonElement element : configObject.get("Waypoints").getAsJsonArray()) {
                ...
                if(Solstice.WAYPOINT_MANAGER.contains(args[0])) continue;
                Solstice.WAYPOINT_MANAGER.add(args[0], new Vec3d(Integer.parseInt(args[1]), Integer.parseInt(args[2]), Integer.parseInt(args[3])), args[4], args[5]);
            }
        }
    }

    public void loadGeneral() throws IOException {
        if (!FileUtils.fileExists(Solstice.MOD_NAME + "/General.json")) return;
        @Cleanup InputStream stream = Files.newInputStream(Paths.get(Solstice.MOD_NAME + "/General.json"));

        JsonObject configObject;
        try {
            configObject = JsonParser.parseReader(new InputStreamReader(stream)).getAsJsonObject();
        } catch (IllegalStateException exception) {
            Solstice.LOGGER.error("Failed to load the client's General configuration!", exception);
            Solstice.CHAT_MANAGER.await("The General configuration has not been loaded properly. Read the stacktrace for more information.");
            return;
        }

        if (configObject.has("Config")) currentConfig = configObject.get("Config").getAsString();
        if (configObject.has("Prefix")) Solstice.COMMAND_MANAGER.setPrefix(configObject.get("Prefix").getAsString());
        if (configObject.has("Friends")) {
            for (JsonElement element : configObject.get("Friends").getAsJsonArray()) {
                if (Solstice.FRIEND_MANAGER.contains(element.getAsString())) continue;
                Solstice.FRIEND_MANAGER.add(element.getAsString());
            }
        }
        ...
        if (configObject.has("Macros")) {
            for (JsonElement element : configObject.get("Macros").getAsJsonArray()) {
                try {
                    ...
                    Solstice.MACRO_MANAGER.add(message, key);
                } catch (NumberFormatException exception) {
                    Solstice.LOGGER.error("Failed to load the " + element.getAsString() + " macro!", exception);
                    Solstice.CHAT_MANAGER.await("The " + element.getAsString() + " macro has failed to load. Read the stacktrace for more information.");
                    continue;
                }
            }
        }
    ...
    public void saveGeneral() throws IOException {
        FileUtils.resetFile(Solstice.MOD_NAME + "/General.json");

        JsonObject configObject = new JsonObject();
        configObject.add("Config", new JsonPrimitive(currentConfig));
        configObject.add("Prefix", new JsonPrimitive(Solstice.COMMAND_MANAGER.getPrefix()));

        JsonArray friendsArray = new JsonArray();
        Solstice.FRIEND_MANAGER.getFriends().forEach(friendsArray::add);
        configObject.add("Friends", friendsArray);

        JsonArray macroArray = new JsonArray();
        Solstice.MACRO_MANAGER.getMacros().forEach((key, value) -> {
            macroArray.add(value + ":" + key);
        });
        configObject.add("Macros", macroArray);

        @Cleanup OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(Solstice.MOD_NAME + "/General.json"), StandardCharsets.UTF_8);
    ...
    public void saveWaypoints() throws IOException {
        FileUtils.resetFile(Solstice.MOD_NAME + "/Waypoints.json");

        JsonObject configObject = new JsonObject();

        JsonArray waypointArray = new JsonArray();
        for(WaypointManager.Waypoint waypoint : Solstice.WAYPOINT_MANAGER.getWaypoints()) {
        ...
        @Cleanup OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(Solstice.MOD_NAME + "/Waypoints.json"), StandardCharsets.UTF_8);
        writer.write(new GsonBuilder().setPrettyPrinting().create().toJson(JsonParser.parseString(configObject.toString())));
    }

    public void loadModules(String config) throws IOException {
        if (!FileUtils.fileExists(Solstice.MOD_NAME + "/Configs/" + config + ".json")) return;
        @Cleanup InputStream stream = Files.newInputStream(Paths.get(Solstice.MOD_NAME + "/Configs/" + config + ".json"));

        JsonObject configObject;
        try {
            configObject = JsonParser.parseReader(new InputStreamReader(stream)).getAsJsonObject();
        } catch (IllegalStateException exception) {
            Solstice.LOGGER.error("Failed to load the client's Module configuration!", exception);
            Solstice.CHAT_MANAGER.await("The configuration for the Modules has not been loaded properly. Read the stacktrace for more information.");
            return;
        }
        ...
        for (Module module : Solstice.MODULE_MANAGER.getModules()) {
    ...
    public void saveModules(String config) throws IOException {
        FileUtils.resetFile(Solstice.MOD_NAME + "/Configs/" + config + ".json");
        ...
        @Cleanup OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(Solstice.MOD_NAME + "/Configs/" + config + ".json"), StandardCharsets.UTF_8);
        writer.write(new GsonBuilder().setPrettyPrinting().create().toJson(JsonParser.parseString(configObject.toString())));
