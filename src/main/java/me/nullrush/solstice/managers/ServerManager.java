...
import me.nullrush.solstice.modules.impl.miscellaneous.FastLatencyModule;
...
public class ServerManager implements IMinecraft {
        ...
        if (event.getPacket() instanceof PlayerListS2CPacket packet) {
            if (packet.getActions().contains(PlayerListS2CPacket.Action.ADD_PLAYER)) {
                for(PlayerListS2CPacket.Entry entry : packet.getPlayerAdditionEntries()) {
                    Solstice.EVENT_HANDLER.post(new PlayerConnectEvent(entry.profile().getId()));
                }
            }
        } else if(event.getPacket() instanceof PlayerRemoveS2CPacket packet) {
            for(UUID id : packet.profileIds()) {
                Solstice.EVENT_HANDLER.post(new PlayerDisconnectEvent(id));
            }
        }
    }
    ...
    public int getPing() {
        if (Solstice.MODULE_MANAGER.getModule(FastLatencyModule.class).isToggled()) {
            return Solstice.MODULE_MANAGER.getModule(FastLatencyModule.class).getLatency();
        }
