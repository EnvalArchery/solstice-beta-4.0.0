...
import me.nullrush.solstice.mixins.accessors.EntityAccessor;
...
import me.nullrush.solstice.utils.rotations.Rotation;
...
public class RotationManager implements IMinecraft {
    ...
    public void onUpdateVelocity(UpdateVelocityEvent event) {
        ...
        if (!Solstice.MODULE_MANAGER.getModule(RotationsModule.class).movementFix.getValue()) return;
        if (rotation == null) return;
    ...
    public void packetRotate(float yaw, float pitch) {
        ...
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(Solstice.POSITION_MANAGER.getServerX(), Solstice.POSITION_MANAGER.getServerY(), Solstice.POSITION_MANAGER.getServerZ(), yaw, pitch, Solstice.POSITION_MANAGER.isServerOnGround(), mc.player.horizontalCollision));
    }
