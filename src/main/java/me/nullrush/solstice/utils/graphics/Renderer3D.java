...
import me.nullrush.solstice.mixins.accessors.WorldRendererAccessor;
...
public class Renderer3D implements IMinecraft {
    ...
    public static void renderText(MatrixStack matrices, String text, double x, double y, double z, float scaling, boolean background, Color color) {
        ...
        if (background) Renderer2D.renderQuad(matrices, -Solstice.FONT_MANAGER.getWidth(text) / 2.0f - 2, -Solstice.FONT_MANAGER.getHeight() - 2, Solstice.FONT_MANAGER.getWidth(text) / 2.0f + 2, 1, new Color(0, 0, 0, 100));
        Solstice.FONT_MANAGER.drawTextWithShadow(matrices, text, -Solstice.FONT_MANAGER.getWidth(text) / 2, -Solstice.FONT_MANAGER.getHeight(), vertexConsumers, color);
