package me.nullrush.solstice.utils.graphics;

import me.nullrush.solstice.Solstice;
import me.nullrush.solstice.modules.impl.visuals.NoInterpolationModule;
import me.nullrush.solstice.utils.IMinecraft;
import me.nullrush.solstice.utils.mixins.ILivingEntityRenderer;
import me.nullrush.solstice.utils.mixins.IMultiPhase;
import me.nullrush.solstice.utils.mixins.IMultiPhaseParameters;
...
public class ModelRenderer implements IMinecraft {
    ...
    public static void renderModel(Entity entity, boolean staticEntity, float scale, float tickDelta, Render render) {
        ...
        if (!Solstice.MODULE_MANAGER.getModule(NoInterpolationModule.class).isToggled()) ModelRenderer.offset = new Vec3d(MathHelper.lerp(tickDelta, entity.lastRenderX, entity.getX()), MathHelper.lerp(tickDelta, entity.lastRenderY, entity.getY()), MathHelper.lerp(tickDelta, entity.lastRenderZ, entity.getZ()));
        ...
        if (renderer instanceof LivingEntityRenderer<?, ?, ?> && renderState instanceof LivingEntityRenderState state) {
            ((ILivingEntityRenderer) renderer).solstice$render(state, matrices, CustomVertexConsumerProvider.INSTANCE, 15);
        }
    ...
    private static class CustomVertexConsumerProvider implements VertexConsumerProvider {
        ...
        public VertexConsumer getBuffer(RenderLayer layer) {
            if (layer instanceof IMultiPhase phase && ((IMultiPhaseParameters) (Object) phase.solstice$getParameters()).solstice$getTarget() == RenderLayer.ITEM_ENTITY_TARGET) {
