package me.nullrush.solstice.utils.text;

import me.nullrush.solstice.Solstice;
import me.nullrush.solstice.mixins.accessors.StyleAccessor;
import me.nullrush.solstice.mixins.accessors.TextColorAccessor;
...
public class FormattingUtils {
    ...
    public static List<String> wrapText(String text, int width) {
        ...
        for(String word : words) {
            if(Solstice.FONT_MANAGER.getWidth(current) + Solstice.FONT_MANAGER.getWidth(word) <= width) {
                current += word + " ";
            } else {
        ...
        if(Solstice.FONT_MANAGER.getWidth(current) > 0) wrappedText.add(current);
