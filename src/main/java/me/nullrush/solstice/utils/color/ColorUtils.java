package me.nullrush.solstice.utils.color;
...
public class ColorUtils {
    ...
    public static Color getGlobalColor() {
        return Solstice.MODULE_MANAGER.getModule(ColorModule.class).color.getColor();
    }

    public static Color getGlobalColor(int alpha) {
        return getColor(Solstice.MODULE_MANAGER.getModule(ColorModule.class).color.getColor(), alpha);
    }
    ...
    public static Color getRainbow(int alpha) {
        return getRainbow(Solstice.MODULE_MANAGER.getModule(ColorModule.class).rainbowSpeed.getValue().longValue(), Solstice.MODULE_MANAGER.getModule(ColorModule.class).rainbowSaturation.getValue().floatValue() / 100.0f, Solstice.MODULE_MANAGER.getModule(ColorModule.class).rainbowBrightness.getValue().floatValue() / 100.0f, alpha);
    }
    ...
    public static Color getOffsetWave(Color color, long index) {
        return getWave(color, Solstice.MODULE_MANAGER.getModule(ColorModule.class).rainbowSpeed.getValue().longValue(), 255, index);
    }
    ...
    public static Color getPulse(Color color, long speed) {
        ...
        double sin = Math.sin(2 * Math.PI * (speed/20f) * ((System.currentTimeMillis() - Solstice.UPTIME)/1000f));
