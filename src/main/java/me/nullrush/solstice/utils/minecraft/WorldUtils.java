...
import me.nullrush.solstice.modules.impl.core.RotationsModule;
...
public class WorldUtils implements IMinecraft {
    ...
    public static void placeBlock(BlockPos position, Direction direction, Hand hand, Runnable runnable, boolean rotate, boolean crystalDestruction, boolean render) {
        ...
        float prevYaw = Solstice.ROTATION_MANAGER.getServerYaw();
        float prevPitch = Solstice.ROTATION_MANAGER.getServerPitch();

        if (rotate) Solstice.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(vec3d.getX(), vec3d.getY(), vec3d.getZ()));
        ...
        if (rotate && Solstice.MODULE_MANAGER.getModule(RotationsModule.class).snapBack.getValue()) Solstice.ROTATION_MANAGER.packetRotate(prevYaw, prevPitch);

        Solstice.WORLD_MANAGER.getPlaceTimer().reset();
        ...
        if (render) {
            ...
            if(!Solstice.RENDER_MANAGER.renderPositions.contains(renderPosition)) Solstice.RENDER_MANAGER.renderPositions.add(renderPosition);
        }
    }
