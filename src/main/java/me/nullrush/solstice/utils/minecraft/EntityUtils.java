...
public class EntityUtils implements IMinecraft {
    ...
    public static boolean isBot(PlayerEntity player) {
        if (Solstice.MODULE_MANAGER.getModule(FakePlayerModule.class).isToggled() && player == Solstice.MODULE_MANAGER.getModule(FakePlayerModule.class).getPlayer()) {
            return false;
        }
    ...
    public static double getSpeed(Entity entity, SpeedUnit unit) {
        ...
        if (unit == SpeedUnit.KILOMETERS) return (speed * 3.6 * Solstice.WORLD_MANAGER.getTimerMultiplier()) * 20;
        else return speed / 0.05 * Solstice.WORLD_MANAGER.getTimerMultiplier();
    }
