package me.nullrush.solstice.utils.font;
...
import me.nullrush.solstice.modules.impl.miscellaneous.NameProtectModule;
...
public class FontRenderer implements Closeable, IMinecraft {
    ...
    public FontRenderer(Font[] fonts, float size) {
        this(fonts, size, true, true);
    }

    public FontRenderer(Font[] fonts, float size, boolean antiAliasing, boolean fractionalMetrics) {
        this(fonts, size, 256, 5, antiAliasing, fractionalMetrics);
    }

    public FontRenderer(Font[] fonts, float size, int charactersPerPage, int paddingBetweenCharacters) {
        this(fonts, size, charactersPerPage, paddingBetweenCharacters, true, true);
    }

    public FontRenderer(Font[] fonts, float size, int charactersPerPage, int paddingBetweenCharacters, boolean antiAliasing, boolean fractionalMetrics) {
    ...
    public void drawText(MatrixStack matrices, OrderedText text, float x, float y, int color, boolean dropShadow) {
        ...
        if (Solstice.MODULE_MANAGER != null && Solstice.MODULE_MANAGER.getModule(FontModule.class).isToggled()) {
            x += Solstice.MODULE_MANAGER.getModule(FontModule.class).xOffset.getValue().floatValue();
            y += Solstice.MODULE_MANAGER.getModule(FontModule.class).yOffset.getValue().floatValue();
        }
    ...
    public float getTextWidth(String text) {
        NameProtectModule module = Solstice.MODULE_MANAGER.getModule(NameProtectModule.class);
    ...
    private Glyph locateGlyph(char glyph) {
        ...
        GlyphMap map = new GlyphMap(this.fonts, (char) base, (char) (base + charsPerPage), padding, antiAliasing, fractionalMetrics);
        LOCK.writeLock().lock();
