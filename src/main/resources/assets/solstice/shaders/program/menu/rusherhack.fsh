...
const vec2 ch_start = vec2(ch_space.x * -5., 3.0);
...
#define s_question ddigit(0x420E);
...
float dseg(vec2 p0, vec2 p1) {
    ...
    return distance(cp, clamp(cp, vec2(0.0), vec2(distance(p0, p1), 0.0)));
}
...
void main(void) {
    ...
    float vignette = 1.0 - length(uv);
    uv *= 18.0;
    uv -= vec2(-3.0, 1.0);

    vec3 ch_color = hsv2rgb_smooth(vec3(time * 0.4 + uv.y * 0.1, 0.5, 1.0));
    vec3 bg_color = vec3(vignette * 0.4, vignette * 0.2, vignette * 0.1);

    uv.x += 0.5 + sin(time + uv.y * 0.7) * 0.5;
    ch_pos = ch_start;

    R U S H E R _ H A C K nl1

    vec3 color = mix(ch_color, bg_color, 1.0 - (0.08 / d * 2.0));
    RusherHack_FragColor = vec4(color, 1.0);
}
